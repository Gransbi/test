...
char **tokenizeBySpace(char *input)
{
  ...
  int index = 0;
  char *currentPosition = trim(input);
  char *nextSpace;

  while (*currentPosition)
  {
    nextSpace = strchr(currentPosition, ' ');
    if (!nextSpace)
    {
      tokens[index++] = strdup(currentPosition);
      break;
    }
    tokens[index++] = strndup(currentPosition, nextSpace - currentPosition);
    currentPosition = trim(nextSpace);
  }
...
void handleRedirection(Args *arg)
{
  for (int i = 0; arg->args[i]; i++)
  {
    ...
    if (operator)
    {
      ...
      {
        fd = open(arg->args[i + 1], O_WRONLY | O_CREAT | O_TRUNC);
      }
      else if (strcmp(operator, ">>") == 0)
      {
        fd = open(arg->args[i + 1], O_WRONLY | O_CREAT | O_APPEND);
      }
      else if (strcmp(operator, "<") == 0)
      {
